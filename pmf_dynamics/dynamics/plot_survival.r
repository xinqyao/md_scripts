...
#fit <- "single"   
#fit <- "power"   
fit <- c("single", "single", "multiple")
#bb <- c(-1.0, -0.1, -0.01) ## exponentials
bbs <- list("3.5"=c(-0.01, -0.001),
           "4.2"=c(-0.01, -0.001),
           "4.9"=c(-0.01, -0.001)) ## exponentials
...
#      out <- lm(log(prob)~time+0, data=mydat)
      ...
      ssq <- c(ssq, 1-summary(out)$r.squared)
      ...
      # use nls to get parameter errors
      Ks <- out$b
      As <- out$a
      startvals <- c(As, Ks)
      names(startvals) <- c(paste("a", 1:length(As), sep=""), paste("b", 1:length(Ks), sep=""))
      out <- switch(as.character(length(Ks)), 
         "2"= nls(prob~a1*exp(b1*time)+a2*exp(b2*time), data=mydat, start=startvals, weights=ww),
         "3"= nls(prob~a1*exp(b1*time)+a2*exp(b2*time)+a3*exp(b3*time), data=mydat, start=startvals, weights=ww),
         "4"= nls(prob~a1*exp(b1*time)+a2*exp(b2*time)+a3*exp(b3*time)+a4*exp(b4*time), data=mydat, start=startvals, weights=ww))
      
      Ks <- abs(coef(out)[grep("^b", names(coef(out)))])
      As <- abs(coef(out)[grep("^a", names(coef(out)))])
#      Ks <- abs(out$b)
#      As <- abs(out$a)
      ...
      for(i in 1:length(As)) {
         myerr <- myerr + (Ks[i]/sum(As) - sum(As*Ks)/(sum(As)^2))^2*summary(out)$coefficients[paste0("a",i), "Std. Error"]^2
      }
      for(i in 1:length(Ks)) {
         myerr <- myerr + (As[i]/sum(As))^2*summary(out)$coefficients[paste0("b",i), "Std. Error"]^2
      }
      
      err <- c(err, sqrt(myerr))
